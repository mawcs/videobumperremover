...
- [`docs/decisions/`](docs/decisions/) — ADRs: 0001 stack (superseded by 0002), 0002 stack
  ...
  (implemented and live-verified 2026-08-03 — `SampleFrames` only, `SampleKeyframes` stayed
  CLI-only; most real installs lack the shared FFmpeg libraries native decode needs, so it's
  currently a no-op for them until VBR.CLI gains its own acquisition path, see its Open questions).

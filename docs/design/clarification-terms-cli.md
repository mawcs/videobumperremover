...
There are a variety of cases of implicit portablility that naturally fall out of users and their own interactions with their systems and their media.
...
VBR will want to handle all of the cases; at least in a policy or strategy sense if not outright accomodations for the cases.

1. VBR should accept the contents of a "library database" or "bumper catalog" that has been moved.
2. VBR may support a library that has moved. (Currently, this is not supported, but there is motivation to support it. Making a user rescan just because the absolute path changed is a bit too much.)

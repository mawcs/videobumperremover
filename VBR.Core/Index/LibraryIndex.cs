...
/// <summary>
/// A named library's persisted, cached fingerprint index — docs/iterativeplan.md, "Library scan —
/// cached fingerprint index." One physical file per named library (decision 13), never VDF's own
/// <c>FileEntry</c>/<c>ScannedFiles.db</c> (decision 2): VDF's <c>grayBytes</c>/<c>PHashes</c>
/// dictionaries are keyed to VDF's own uniform sampling positions, and reusing them for this
/// project's non-uniform edge/sparse samples wouldn't actually feed VDF's own dedup scan (see
/// docs/iterativeplan.md's 2026-07-24 entry for the full reasoning) — so this is a wholly separate
/// store instead.
///
...
/// index files already in the wild.
/// </summary>
...
public sealed partial class LibraryIndex {
	...
	/// <summary>The <c>EdgeDensityProfile</c> this index was last scanned under — recorded so a
	...
	/// <summary>Keyed by the entry's own <see cref="LibraryIndexEntry.Path"/>. Always look up via
	/// <see cref="LibraryIndexKey.Normalize"/> — MemoryPack round-tripping a <c>Dictionary</c> is
	...
	[MemoryPackOrder(5)]
	public Dictionary<string, LibraryIndexEntry> Entries { get; set; } = new();
}

/// <summary>Normalizes a path for use as a <see cref="LibraryIndex.Entries"/> key — Windows paths
...
public static class LibraryIndexKey {
